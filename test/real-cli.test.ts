...
const enabled = process.env.PI_GEMINI_ACP_REAL === "1";
...
describe.skipIf(!enabled)("Google Antigravity ACP contract", () => {
	it("resolves the Antigravity server and completes ACP initialize", async () => {
		const entry = resolveBundledGeminiEntry();
		expect(entry).toMatch(/agy_acp_server\.par$/u);

		const connection = new GeminiAcpConnection({ cwd: process.cwd(), initializeTimeoutMs: 45_000 });
		try {
